...
def scrape_pollen_data(city=None):
    ...
    try:
        ...
        if not pollen_items:
            logging.warning("Cannot extract pollen data from webpage, using default pollen type list")
            
            # Common pollen types in German regions
            default_pollen_types = [
                'Ambrosia', 'Ampfer', 'Beifuß', 'Birke', 'Buche', 'Erle', 
                'Esche', 'Gräser', 'Hasel', 'Pappel', 'Roggen', 'Ulme', 
                'Wegerich', 'Weide'
            ]
            
            default_items = [{'type': pollen_type, 'concentration': '0'} for pollen_type in default_pollen_types]
...
def format_email_content(data, language='en'):
    ...
    Args:
        ...
        language (str): Email language, supports 'en' (English) and 'de' (German)
...
def main(args=None):
    ...
    parser.add_argument('--language', type=str, choices=['en', 'de'], default='en', 
                        help='Email language: en (English), de (German)')
